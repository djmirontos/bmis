...
function NewOfficialPageInner() {
  ...
  function validateForm(): boolean {
    ...
    if (!isTanod) {
      ...
      if (form.term_start && form.term_end && form.term_end <= form.term_start)
        newErrors.term_end = 'Term end date must be after term start date.'
    }
  ...
  async function handleSave() {
    ...
    if (signatureFile && !isTanod) {
      ...
      const { data: uploadData, error: uploadError } = await supabase.storage
        .from('signatures')
        .upload(fileName, signatureFile, { contentType: 'image/png', upsert: false })
      if (uploadError) {
    ...
    if (isTanod) {
      const { error } = await supabase.from('barangay_tanod').insert({
      ...
      const { error } = await supabase.from('sk_officials').insert({
        full_name: form.full_name.trim(),
    ...
    if (insertError) {
      setSubmitError('Error saving: ' + insertError.message)
      setSaving(false)
      return
    }
    ...
    await supabase.from('audit_logs').insert({
      ...
      notes: `New ${isTanod ? 'tanod' : isSK ? 'SK official' : 'official'} added: ${form.full_name}`,
    })
  ...
  const pageTitle = isTanod
    ? 'Add Barangay Tanod'
    : isSK ? 'Add SK Official' : 'Add Barangay Official'
    ...
    <div className={styles.container}>
      ...
      <Section title="Basic Information">
        <div className={styles.fieldRow}>
          ...
          {!isTanod && (
            <Field label="Committee Assignment" half>
              <select
            ...
            <>
              <Field label="Term Start" required error={errors.term_start} half>
                <input
                  ...
                  onBlur={e => {
                    if (!e.target.value)
                      setFieldError('term_start', 'Term start date is required.')
              ...
              <Field label="Term End" required error={errors.term_end} half>
                <input
                  ...
                  onBlur={e => {
                    if (!e.target.value) {
                      setFieldError('term_end', 'Term end date is required.')
      ...
      {!isTanod && (
        <Section
          ...
          note="Upload a PNG signature image with transparent background. This will be used automatically on printed clearances and certificates. Maximum file size: 2MB."
        >
          {!signatureFile ? (
      ...
      <div className={styles.actions}>
        <Link href="/dashboard/officials" className={styles.cancelButton}>
          Cancel
        </Link>
        <button
          onClick={handleSave}
